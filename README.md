...
| [dev-workflow/dw_ticket_loop.py](dev-workflow/dw_ticket_loop.py) | Manual `dw-ticket-loop plan/run/listen` CLI; the Docker orchestrator can supervise `listen` independently of scheduled queue pickup via roster `command_listener: true` |
| [docs/jev-shadow-pilot.md](docs/jev-shadow-pilot.md) | Opt-in Jev intent comparison for the Telegram listener; never changes live routing |
